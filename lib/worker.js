...
Worker.prototype.run = function(outCallback, callback) {
  ...
  var outputPrefix = '#' + self.options.workerIndex + ' ';
  var finished = false;
  ...
  function done(err, report) {
    ...
    Debug(outputPrefix + 'Closing connection');
    self._myriadConnection.close(function() {
      Debug(outputPrefix + 'Closed connection');
      callback(err, report);
    });
  }
  ...
  Debug(outputPrefix + 'Connecting to ' + self.options.myriadServerUrl);
  self._myriadConnection = Myriad({ url: self.options.myriadServerUrl });

  var stdout = [];

  self._myriadConnection.on('connect', function() {
    Debug(outputPrefix + 'Spawning cucumber instance');
  ...
  self._myriadConnection.on('message', function(event) {
    ...
    if (event.event === 'error') {
      ...
      Debug(outputPrefix + 'Error: ' + JSON.stringify(event.error));
      done(event.error);
    }
    else if (event.event === 'child_stdout') {
      Debug(outputPrefix + 'Child stdout: ' + data.toString());
      stdout.push(data);
    }
      ...
      console.error(event.error);
      Debug(outputPrefix + 'Child rrror: ' + JSON.stringify(event.error));
      done(event.error);
    }
    else if (event.event === 'child_close') {
      Debug(outputPrefix + 'Child closed.  Exit code ' + event.code);
      ...
      if (event.code === 0 || event.code === 1) {
        ...
        if (validJson) {
          ...
          done({ message: outputPrefix + "Cucumber outputted invalid JSON.  Exit code " + event.code, exitCode: event.code, stdout: stdout });
        }
      }
      else {
        done({ message: outputPrefix + "Cucumber returned a failure exit code.  Exit code " + event.code, exitCode: event.code, stdout: stdout });
      }
    }
  });
}
